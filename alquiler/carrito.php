                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody>
                            ...
                            foreach ($carrito as $indice => $item):
                                // Convertimos a float para evitar errores de operandos
                                $subtotal = isset($item['total']) ? (float) $item['total'] : 0;
                                $gran_total += $subtotal;
                                ?>
                                <tr class="border-b">
                                    <td class="py-4 font-bold"><?php echo htmlspecialchars($item['titulo'] ?? 'Libro'); ?></td>
                                    <td class="text-sm text-slate-600">
                                        Del <?php echo htmlspecialchars($item['fecha_inicio'] ?? '---'); ?> 
                                        al <?php echo htmlspecialchars($item['fecha_fin'] ?? '---'); ?>
                                    </td>
                                    <td class="font-bold text-cyan-600">$<?php echo number_format($subtotal, 2); ?></td>
                                    <td>
